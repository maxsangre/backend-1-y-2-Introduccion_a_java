 ...
 */
public class servRuleta02 {
      ArrayList<entiJugador02> jugadoresList = new ArrayList<>();

    public void llenarJuego(ArrayList<entiJugador02> jugadoresList, entiRevolver02 revolver) {
        for (int i = 0; i < 6; i++) {
           entiJugador02 playerMax = new entiJugador02();
            playerMax.setId((i + 1));
            playerMax.setNombre("Jugador.Max " + (i + 1));
            jugadoresList.add(playerMax);
        }
        llenarRevolver(revolver);
    }

    public boolean mojar(entiRevolver02 revolverObj) {
    ...
    public boolean disparo(entiJugador02 jugador, entiRevolver02 revolverObj) {
        boolean seMurio = false;
    ...
    public boolean ronda(entiJugador02 jugador, entiRevolver02 revolverObj) {
    ...
    public void llenarRevolver(entiRevolver02 revolverObj) {
        Random random = new Random();
        revolverObj.setPosActual(random.nextInt(6) + 1);
        revolverObj.setPosAgua(random.nextInt(6) + 1);
    }

    public void siguienteChorro(entiRevolver02 revolverObj) {
